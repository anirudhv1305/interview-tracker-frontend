...
import { AddRounded, CheckCircleRounded, CloseRounded, EditRounded, RadioButtonCheckedRounded, RadioButtonUncheckedRounded, ScheduleRounded, SearchRounded, UpdateRounded } from '@mui/icons-material';
...
const blank = { companyName: '', role: '', appliedDate: new Date().toISOString().slice(0, 10), status: 'PENDING', totalRounds: 1, currentRoundNumber: 1, currentRoundName: 'OA', notes: '' };
const names = ['OA', 'Group Discussion', 'Coding', 'TR', 'HR'];
...
export default function Status({ onChanged }) {
  ...
  return <Stack spacing={3}>
    <PageHeader title="Status" description="Track applications and recruitment progress." actions={<Button variant="contained" startIcon={<AddRounded />} onClick={() => { setChosen(null); setForm(blank); setEdit(true); }}>Add application</Button>} />
    ...
    <Dialog open={Boolean(chosen) || edit} onClose={close} fullWidth maxWidth="sm" PaperProps={{ sx: { maxHeight: 'calc(100dvh - 32px)' } }}>
      ...
      <DialogContent dividers>{edit ? <Stack spacing={2}>
        ...
        <RoundBuilder totalRounds={form.totalRounds} rounds={form.plannedRounds?.length ? form.plannedRounds : Array.from({ length: form.totalRounds }, (_, index) => index === form.currentRoundNumber - 1 ? form.currentRoundName : (names[index] || 'OA'))} showCurrent lockCurrentRound={Boolean(form.plannedRounds?.length)} currentRoundNumber={form.currentRoundNumber} currentRoundName={form.currentRoundName || ''} onChange={(totalRounds, rounds) => setForm({ ...form, totalRounds, currentRoundNumber: Math.min(form.currentRoundNumber, totalRounds), currentRoundName: rounds[Math.min(form.currentRoundNumber - 1, totalRounds - 1)] || form.currentRoundName })} onCurrentChange={(currentRoundNumber, currentRoundName) => setForm({ ...form, currentRoundNumber, currentRoundName: form.plannedRounds?.[currentRoundNumber - 1] || currentRoundName })} />
      ...
      <DialogActions sx={{ p: 2 }}>{edit ? <><Button onClick={() => chosen ? (setForm({ ...blank, ...chosen }), setEdit(false)) : close()}>Cancel</Button><Button variant="contained" onClick={save}>Save changes</Button></> : <><Button onClick={close}>Close</Button><Button variant="contained" startIcon={<EditRounded />} onClick={() => setEdit(true)}>Edit</Button></>}</DialogActions>
    </Dialog>
  </Stack>;
}
